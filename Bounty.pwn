...
#define BOUNTY  "%s.ini"
#define bountydialog 13078
#define strcpy(%0,%1) \
            strcat((%0[0] = '\0', %0), %1)

forward LoadBounty_Data(playerid, name[], value[]);
forward HideText();
...
stock Path(playerid)
{
    ...
    format(string,sizeof(string),BOUNTY,playername);
    return string;
}
...
public OnFilterScriptInit()
{
    ...
    TextDrawBoxColor(BBox, 0x000000AA);
...
public LoadBounty_Data(playerid, name[], value[])
{
	INI_Int("Bounty", bInfo[playerid][Hitman]);
	return 1;
}
...
public OnPlayerConnect(playerid)
{
    ...
    {
        INI_ParseFile(Path(playerid), "LoadBounty_%s", .bExtra = true, .extra = playerid);
        new File[256];
...
{
	new INI:File = INI_Open(Path(playerid));
    INI_WriteInt(File, "Bounty", bInfo[playerid][Hitman]);
    INI_Close(File);
	return 1;
}
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ...
    {
        if(bInfo[playerid][BountyOn] == 1)
        {
            GivePlayerMoney(killerid, bInfo[playerid][Hitman]);
            new string[128];
            format(string, sizeof(string), "Bounty: $%i", bInfo[playerid][Hitman]);
            GameTextForPlayer(playerid, string, 3000, 3);
            SetTimer("HidePayout", 3000, 0);
            bInfo[playerid][BountyOn] = 0;
            return 1;
        }
    }    
	return 1;
...
{
    new id, amount;
    if(sscanf(params, "ui", id, amount)) return SendClientMessage(playerid, -1, "Bounty on player: /hit <PlayerID> <Amount>");
    ...
    bInfo[id][Hitman] = bInfo[id][Hitman] + amount;
    bInfo[id][BountyOn] = 1;

    GivePlayerMoney(playerid, -amount);
    new string[128];
    format(string, sizeof(string), "%s has set bounty on %s: $%i", GetName(playerid), GetName(id), bInfo[id][Hitman]);
...
{
    new tmpbounty = 0;
    SendClientMessage(playerid,0xFF5050FF,"---Bounty List---");
    foreach(new i : Player)
    {
        if(bInfo[i][BountyOn] > 0)
        {
            tmpbounty++;
            new tmpbountystr[50];
            format(tmpbountystr,sizeof(tmpbountystr),"{9999FF}%s(%d): {00FF00}$%i\n", GetName(i), i, bInfo[i][Hitman]);
            SendClientMessage(playerid,0x9999FFAA,tmpbountystr); 
        }
    }
    if(tmpbounty == 0)
    {
        SendClientMessage(playerid,0xFF5050FF, "There is no bounties currently");
    }   
    return 1;
}
...
CMD:position(playerid, params[])
{
    ...
    format(string, sizeof(string), "{%06x}%s {FFFFFF}is located at {00FF00}%s", GetPlayerColor(playerid) >>> 8, GetName(playerid), zone);
